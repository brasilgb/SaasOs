...
class FiscalDocumentController extends Controller
{
    ...
    public function update(Request $request, FiscalSetting $fiscalSetting): RedirectResponse
    {
        ...
        $data = $request->validate([
            ...
            'nfse_mode' => [Rule::requiredIf(fn () => $request->boolean('enabled') && $request->boolean('nfse_enabled')), 'nullable', Rule::in(['municipal', 'national'])],
            'nfse_simple_option' => [Rule::requiredIf(fn () => $request->boolean('enabled') && $request->boolean('nfse_enabled') && $request->input('nfse_mode') === 'national'), 'nullable', Rule::in([1, 2, 3])],
            'nfse_special_tax_regime' => [Rule::requiredIf(fn () => $request->boolean('enabled') && $request->boolean('nfse_enabled') && $request->input('nfse_mode') === 'national'), 'nullable', Rule::in([0, 1, 2, 3, 4, 5, 6, 9])],
            'nfse_operation_indicator' => [Rule::requiredIf(fn () => $request->boolean('enabled') && $request->boolean('nfse_enabled') && $request->input('nfse_mode') === 'national'), 'nullable', 'digits:6'],
            ...
            'nfse_ibs_cbs_situation' => [Rule::requiredIf(fn () => $request->boolean('enabled') && $request->boolean('nfse_enabled') && $request->input('nfse_mode') === 'national'), 'nullable', 'digits:3'],
            'nfse_ibs_cbs_classification' => [Rule::requiredIf(fn () => $request->boolean('enabled') && $request->boolean('nfse_enabled') && $request->input('nfse_mode') === 'national'), 'nullable', 'digits:6'],
            'default_nfe_series' => [Rule::requiredIf(fn () => $request->boolean('enabled') && $request->boolean('nfe_enabled')), 'nullable', 'string', 'max:20'],
            'default_nfse_series' => [
                Rule::requiredIf(fn () => $request->boolean('enabled') && $request->boolean('nfse_enabled')),
                'nullable',
                Rule::when($request->input('nfse_mode') === 'national', ['integer', 'between:1,49999'], ['string', 'max:20']),
            ],
            ...
            'nfse_mode' => 'modalidade da NFS-e',
            'nfse_simple_option' => 'opção do Simples Nacional',
            'nfse_special_tax_regime' => 'regime especial tributário municipal',
            'nfse_operation_indicator' => 'indicador da operação IBS/CBS',
            ...
            'nfse_ibs_cbs_situation' => 'CST do IBS/CBS da NFS-e',
            'nfse_ibs_cbs_classification' => 'classificação tributária do IBS/CBS da NFS-e',
