...
export const columns: ColumnDef<Order>[] = [
  ...
  },
  {
    ...
    header: () => <div className="text-left pl-3">Recebimento</div>,
    cell: ({ row }) => {
  ...
  },
  {
    ...
    header: () => <div className="text-left pl-3">Equipamento</div>,
  },
  {
    accessorKey: "model",
    header: () => <div className="text-left pl-3">Modelo</div>,
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="">
          <Badge variant={'default'}>{statusOrdemByValue(status)}</Badge></div>
      )
    },
  },
  {
    accessorKey: "delivery_date",
    header: () => <div className="text-left pl-3">Entrega</div>,
    cell: ({ row }) => {
      const dtexist = row.original.delivery_date;
      const dataa = new Date(row.getValue("delivery_date"))
      const formatted = moment(dataa).format("DD/MM/YYYY")

      return <div className="text-left font-medium">{dtexist ? formatted : ''}</div>
    },
  },
  {
    accessorKey: " ",
